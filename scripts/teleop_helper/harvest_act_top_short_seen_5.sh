#!/bin/bash
# ACT-only success harvester for Top_Short_Seen_5.
#
...
OUTPUT_ROOT="Datasets/teleop_recovery/top_short_seen_5_act_harvest"
ASSIST_POLICY_DEVICE="${ASSIST_POLICY_DEVICE:-cuda}"
mkdir -p "$OUTPUT_ROOT"

echo "=========================================================================="
echo " ACT HARVEST: Top_Short_Seen_5"
echo "=========================================================================="
...
LEHOME_GLOBAL_KEYBOARD=0 ./third_party/IsaacLab/isaaclab.sh -p -m scripts.dataset_sim record \
    ...
    --score_probe_log /tmp/harvest_act_top_short_seen_5_scores.jsonl \
    ...
    --assist_policy_path outputs/train/act_top_short_aug/checkpoints/050000/pretrained_model \
    --assist_dataset_root Datasets/example/top_short_merged \
    --assist_policy_device "$ASSIST_POLICY_DEVICE" \
    --device cpu \
    2>&1 | tee "/tmp/harvest_act_top_short_seen_5.log"
